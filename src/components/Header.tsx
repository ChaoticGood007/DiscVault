...
export async function Header({ 
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex items-center min-w-0">
            ...
            {children && (
               <div className="flex items-center gap-2 sm:gap-3 pl-3 sm:pl-6 ml-3 sm:ml-6 border-l border-slate-200 min-w-0 shrink">
                 {children}
               </div>
            )}
          </div>
          
          <div className="flex items-center gap-1.5 md:gap-4 shrink min-w-0 bg-white">

              {/* Dynamic Sub-Navigation Links */}
              {actions && (
                <div className="hidden 2xl:flex items-center gap-1 p-1 bg-slate-50 rounded-xl border border-slate-100 overflow-x-auto max-w-full no-scrollbar whitespace-nowrap">
                  {actions}
                </div>
              )}
      ...
      {/* Mobile Actions - Flex Wrap instead of Scroll */}
      {actions && (
        <div className="2xl:hidden w-full border-t border-slate-100 bg-slate-50 px-2 sm:px-4 py-2">
