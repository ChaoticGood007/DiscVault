...
export default async function AllVaultsStatsPage() {
  const stats = await getInventoryStats() // No ID = Global
  ...
  if (!stats) {
    return (
      <div className="text-center py-24 bg-white rounded-[32px] border-4 border-dashed border-slate-100 shadow-inner">
        ...
        <p className="mt-2 text-slate-500 font-medium max-w-sm mx-auto">Add some discs to any vault to see global analytics.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-10">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          { label: 'Global Total', value: stats.totalDiscs },
      ...
      <div className="bg-slate-900 p-8 rounded-[40px] shadow-xl shadow-indigo-100 flex flex-col md:flex-row items-center justify-between gap-8 text-white relative overflow-hidden">
        <div className="relative z-10">
          <span className="text-indigo-300 text-[10px] font-black uppercase tracking-[0.3em]">Global Insights</span>
          <h2 className="text-2xl font-black mt-1">Total Collection Stats</h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-16 relative z-10">
          ...
          <div>
            ...
            <p className="text-[10px] text-indigo-200/60 font-medium mt-1">Dominates your entire collection</p>
          </div>
        </div>
